...
const SBT_ABI = [
  ...
  { name: "mintTo", type: "function", stateMutability: "nonpayable", inputs: [{ name: "to", type: "address" }], outputs: [] },
] as const;
...
export default function SbtMintButton({ onSuccess }: { onSuccess?: () => void }) {
  ...
  async function mint() {
    ...
    try {
      ...
      alert("SBT 铸造提交成功（mint）"); onSuccess?.();
    } catch {
      try {
        // 再尝试 mintTo(address)
        await writeContract({
        ...
        alert("SBT 铸造提交成功（mintTo）"); onSuccess?.();
      } catch (e: any) {
